...
public class BST<T> {
    ...
    public T getRoot() throws NoSuchElementException {
        if (isEmpty()) {
            throw new NoSuchElementException("getRoot: binary search tree is empty!");
        }
        return root.data;
    }
    ...
    private int getSize(Node node) {
        ...
        if (node == null) {
            ...
            // action
            return (1 + getSize(node.left) + getSize(node.right)); // recursively call on the root's left child
            // recursively call on the root's right child
        }
    ...
    public T findMin() throws NoSuchElementException {
        if (isEmpty()) {
            throw new NoSuchElementException("findMin(): binary search tree is empty!");
        }
        return findMin(root);
    }
    ...
    public T findMax() throws NoSuchElementException {
        if (isEmpty()) {
            throw new NoSuchElementException("findMax(): binary search tree is empty!");
        }
        return findMax(root);
    }
    ...
    private T search(T data, Node node, Comparator<T> c) {
        if (c.compare(data, node.data) == 0) // update to use Comparator
        {
            return node.data;
    ...
    public void remove(T data, Comparator<T> c) {
        this.root = remove(data, this.root, c);
    }
    ...
    public ArrayList<T> compileTree() {
        return compileTree(root, new ArrayList<>());
    }

    public ArrayList<T> compileTree(Node node, ArrayList<T> compiledTree) {
        if (node == null)
            return compiledTree;

        compiledTree.add(node.data);

        compileTree(node.left, compiledTree);
        compileTree(node.right, compiledTree);
